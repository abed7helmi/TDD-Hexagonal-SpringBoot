...
function onConnect(btn,id) {
    if(!courbes[id]){
        ...
        var connection = new EventSource("/streamTransactionsBySociete/" + id);  // je vais la cnx vers le RestController
        connection.onmessage=function (resp) { // je recupere les données
